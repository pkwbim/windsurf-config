...
<template>
    <form         <input type="text" v-model="form.name" />
...
- Using `<form>` without preventing default submission (use `<Form>` component or `- Forgetting to check if `<Form>` component is available in your Inertia version